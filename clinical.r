...
dat1=dat[,-c(1,4,11,12,13,14,15,17,18,19)]
...
for(r in 1:5){
  ...
  feature.matrix = feature.matrix[,-10]
  ...
  for(k in 1:K){
    ...
    pc_numeric_pros_train = pros_train_old[,c(3,7,10)]
    pc_categorical_pros_train = pros_train_old[,c(1,2,4,5,6,8,9,10)]
    ...
    colnames(mrna.feature.matrix.mmi) = colnames(pc_numeric_pros_train[1:2])
    for(i in 1:2){
    ...
    if(length(features)<=2){
      x.train=as.data.frame(pros_train[,-c(length(features))])
      colnames(x.train)=features[1]
      x.train[,1]=as.factor(x.train[,1])
      y.train = pros_train$PathStage
      x.valid = as.data.frame(pros_valid[,-c(length(features))])
      colnames(x.valid)=features[1]
      x.valid[,1]=as.factor(x.valid[,1])
      y.valid=pros_valid$PathStage
      feature.matrix[k,colnames(x.train)] = 1
      ...
      pros_fs_train = pros_train[,c(features[1],'PathStage')]
      pros_fs_valid = pros_valid[,c(features[1],'PathStage')]
    }
    else{
      ...
      ctrl <- rfeControl(functions = rfFuncs,
                         ...
                         verbose = FALSE)
      #if(length(vec) <= 2){
      #  x.check=as.matrix(x.train)
      #  x.train=x.check
      #}
      #x.check=as.data.frame(x.train)
